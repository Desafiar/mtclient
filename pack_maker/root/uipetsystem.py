...
AFFECT_DICT = {
		...
		93 : localeInfo.TOOLTIP_APPLY_RESIST_WOLFMAN,
		...
		# item.APPLY_RESIST_MAGIC_REDUCTION : localeInfo.APPLY_RESIST_MAGIC_REDUCTION,
...
class PetSystemMain(ui.ScriptWindow):
	...
	def PetSkillTooltipShow(self, slot):
		if self.arrytooltip[slot][0] > 0:
			...
			if self.arrytooltip[slot][0] != 10 and self.arrytooltip[slot][0] != 17 and self.arrytooltip[slot][0] != 18:
				...
				self.SkillTooltip.AutoAppendTextLine(uiScriptLocale.PET_SYSTEM_15 +" "+ str(tokens2[self.arrytooltip[slot][1]+1]))
			elif self.arrytooltip[slot][0] == 17:
				self.SkillTooltip.AutoAppendTextLine(uiScriptLocale.PET_SYSTEM_16+" " + checkdiv(int(tokens2[self.arrytooltip[slot][1]+1])) + " sec")
			
			self.SkillTooltip.AutoAppendTextLine(uiScriptLocale.PET_SYSTEM_17+" "+tokens[5]+" sec", grp.GenerateColor(1.0, 0.7843, 0.0, 1.0))
	...
	def SetLevel(self, level):
		if int(level) == 41 or int(level) == 81 or int(level) == 82:
	...
	def SetDuration(self, dur, durt):
		...
		if int(dur) > 0:
			...
			self.petlifeg.Hide()
		self.petdur.SetText(str(dur1)+"/"+str(durt1)+" "+uiScriptLocale.PET_SYSTEM_19)
		self.SetAges(str(tmpage)+uiScriptLocale.PET_SYSTEM_18)
	...
	def SetExperience(self, expm, expi, exptot):
		...
		#####
		self.tooltipexp[0].SetText(uiScriptLocale.PET_SYSTEM_20+" : %d of %d" % (expm, totexpm))
		self.tooltipexp[1].SetText(uiScriptLocale.PET_SYSTEM_20+" : %.2f%%" % expmp)
		self.tooltipexp[2].SetText(uiScriptLocale.PET_SYSTEM_20+"I : %d of %d" % (expi, totexpi))
		self.tooltipexp[3].SetText(uiScriptLocale.PET_SYSTEM_20+"I : %.2f%%" % expip)
...
class PetSystemMini(ui.ScriptWindow):
	class TextToolTip(ui.Window):
		...
		def OnRender(self):
			(mouseX, mouseY) = wndMgr.GetMousePosition()
			self.textLine.SetPosition(mouseX+5, mouseY - 60 + self.y)

	def __init__(self, pet_sys, vnum = 0):
		ui.ScriptWindow.__init__(self)
		self.vnum = vnum
		self.pet_main = pet_sys
		self.__LoadWindow()		
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.icon_button = self.GetChild("icon_button")
			self.tooltipexp = []
			for i in range(0,4):
				self.tooltipexp.append(self.TextToolTip(15*i))
				self.tooltipexp[i].Hide()
			...
			self.icon_button.OnMouseLeftButtonUp = lambda : self.__OpenMainBoard()
	...
	def __OpenMainBoard(self):
		if constInfo.PETGUI == 0:
			self.pet_main.Show()
			self.pet_main.SetTop()
			constInfo.PETGUI = 1
		else:
			self.pet_main.Close()
			constInfo.PETGUI = 0
	...
	def SetImageSlot(self, vnum):
		...
		if int(vnum) > 0:
			self.main_slot_img.SetUpVisual("icon/item/"+str(vnum)+".tga")
			self.main_slot_img.SetOverVisual("icon/item/"+str(vnum)+".tga")
			self.main_slot_img.SetDownVisual("icon/item/"+str(vnum)+".tga")
			self.main_slot_img.Show()
			# self.mainicon.Show()
	...
	def OnUpdate(self):
		self.pet_icon_slot_ani_img = self.GetChild("pet_icon_slot_ani_img")
		if constInfo.PETMINIEVO == 0 and constInfo.EVOLUTION == 41:
			self.pet_icon_slot_ani_img.Show()
		elif constInfo.PETMINIEVO == 1 and constInfo.EVOLUTION == 81:
			self.pet_icon_slot_ani_img.Show()
		elif constInfo.PETMINIEVO == 2 and constInfo.EVOLUTION == 82:
			self.pet_icon_slot_ani_img.Show()
		else:
			self.pet_icon_slot_ani_img.Hide()
			
		if TRUE == self.pet_mini_exp_01.IsIn() or TRUE == self.pet_mini_exp_02.IsIn() or TRUE == self.pet_mini_exp_03.IsIn() or TRUE == self.pet_mini_exp_04.IsIn() or TRUE == self.pet_mini_exp_05.IsIn() or TRUE == self.expwind.IsIn(): #@fixmevegas02
			for i in range(0,4):				
				self.tooltipexp[i].Show()
				#(mouseX, mouseY) = wndMgr.GetMousePosition()			
				#self.tooltipexp[i].SetPosition(mouseX,mouseY)
				#vegas_test(0)
		else:
			for i in range(0,4):				
				self.tooltipexp[i].Hide()
