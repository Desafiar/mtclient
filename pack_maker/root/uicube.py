...
class CubeWindow(ui.ScriptWindow):
	...
	def __OverInMaterialSlot(self, trash, resultIndex, col):
		...
		for itemVnum, count in materialInfo[col]:
			item.SelectItem(itemVnum)
			if player.GetItemCountByTT(itemVnum) >= count:
	...
	def Close(self):
		...
		for i in xrange(self.RESULT_SLOT_COUNT):
			...
			self.resultSlots[i].ClearSlot(0)
	...
	def Refresh(self):
		...
		#import chat
	#	chat.AppendChat(1,"%d"%(len(self.cubeMaterialInfos)))
		...
		if len(self.cubeMaterialInfos) < 3:
			self.RESULT_SLOT_COUNT = len(self.cubeMaterialInfos)
		else:
			self.RESULT_SLOT_COUNT = len(self.resultSlots)


		#import chat
		#chat.AppendChat(1,"%d %d"%(self.RESULT_SLOT_COUNT,len(self.cubeMaterialInfos)))
	...
	def __OnSelectEmptySlot(self, selectedSlotPos):
		...
		if isAttached:
			...
			if app.ENABLE_SPECIAL_STORAGE:
				if player.SLOT_TYPE_INVENTORY != attachedSlotType and player.SLOT_TYPE_UPGRADE_INVENTORY != attachedSlotType:
					return
			else:
			...
			if app.ENABLE_SPECIAL_STORAGE:
				...
				net.SendChatPacket("/cube add %d %d %d" % (selectedSlotPos, attachedSlotPos, player.SlotTypeToInvenType(attachedSlotType)))
			else:
