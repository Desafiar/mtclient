...
class AttachMetinDialog(ui.ScriptWindow):
	...
	def Open(self, metinInv, metinItemPos, targetItemPos):
		self.metinItemPos = metinItemPos
		self.targetItemPos = targetItemPos
		self.metinInv = metinInv
		metinIndex = player.GetItemIndex(metinInv, metinItemPos)
		...
		self.oldToolTip.ShowRender(False)
		self.oldToolTip.AddItemData(itemIndex, metinSlot, applyRandomList = apply_random_list)
		...
		if app.ENABLE_APPLY_RANDOM:
			for i in xrange(player.APPLY_RANDOM_SLOT_MAX_NUM):
				apply_random_list.append(player.GetItemApplyRandom(targetItemPos, i))
		self.newToolTip.ShowRender(False)
		self.newToolTip.AddItemData(itemIndex, metinSlot, applyRandomList = apply_random_list)
	...
	def Accept(self):
		if app.ENABLE_SPECIAL_STORAGE:
			net.SendItemUseToItemPacket(self.metinInv, self.metinItemPos, player.INVENTORY, self.targetItemPos)
		else:
