...
class CMouseController(object):
	...
	def AttachObject(self, Owner, Type, SlotNumber, ItemIndex, count = 0):
		...
		try:
			...
			if Type == player.SLOT_TYPE_INVENTORY or\
				...
				Type == player.SLOT_TYPE_COSTUME_INVENTORY or\
				Type == player.SLOT_TYPE_STONE_INVENTORY or\
				Type == player.SLOT_TYPE_CHANGE_INVENTORY:
			...
			if app.ENABLE_IMPROVED_AUTOMATIC_HUNTING_SYSTEM:
				if Type == player.SLOT_TYPE_AUTO:
					if SlotNumber <= player.AUTO_SKILL_SLOT_MAX:
						skillIndex = player.GetSkillIndex(ItemIndex)
						skillGrade = player.GetSkillGrade(ItemIndex)
						self.AttachedIconHandle = skill.GetIconInstanceNew(skillIndex, skillGrade)
					else:
						itemIndex = player.GetItemIndex(ItemIndex)
						item.SelectItem(itemIndex)
						self.AttachedIconHandle = item.GetIconInstance()
			...
			if app.ENABLE_SWITCHBOT:
				if Type == player.SLOT_TYPE_SWITCHBOT:
					item.SelectItem(self.AttachedItemIndex)
					self.AttachedIconHandle = item.GetIconInstance()
			...
			if app.ENABLE_ASLAN_BUFF_NPC_SYSTEM:
				if Type == player.SLOT_TYPE_BUFF_EQUIPMENT:
					item.SelectItem(self.AttachedItemIndex)
					self.AttachedIconHandle = item.GetIconInstance()
	...
	def DeattachObject(self):
		...
		if self.AttachedIconHandle != 0:
			...
			if app.ENABLE_SWITCHBOT:
				if self.AttachedType == player.SLOT_TYPE_SWITCHBOT:
					grpImage.Delete(self.AttachedIconHandle)

			if app.ENABLE_ASLAN_BUFF_NPC_SYSTEM:
				if self.AttachedType == player.SLOT_TYPE_BUFF_EQUIPMENT:
					grpImage.Delete(self.AttachedIconHandle)

			if app.ENABLE_AURA_SYSTEM:
				if self.AttachedType == player.SLOT_TYPE_AURA:
					item.DeleteIconInstance(self.AttachedIconHandle)
