...
class AuraWindow(ui.ScriptWindow):
	...
	def RefreshAuraWindow(self):
		...
		for i in xrange(AuraItemSize):
			...
			if app.WJ_ENABLE_TRADABLE_ICON:
				(isHere, iCell) = player.FindUsingAuraSlot(i)
				self.ItemListIdx.append(iCell)
				# player.FindUsingAuraSlot(player.AURA_SLOT_MAIN)
	...
	if app.WJ_ENABLE_TRADABLE_ICON:
		...
		def CantTradableItem(self, slotIndex):
			...
			if itemIndex:
				...
				if (item.GetItemType() != item.ITEM_TYPE_COSTUME or item.GetItemSubType() != item.COSTUME_TYPE_AURA):
					return True
