...
class GameOptionWindow(ui.BoardWithTitleBar):
		...
		if optionIndex == 0:#Game-Options
			...
			if categoryIndex == 0:#General
				...
				if app.ENABLE_AUTOMATIC_PICK_UP_SYSTEM:
					...
					pickup_filter_child_names = ["rarity_normal", "rarity_uncommun","rarity_rare","rarity_epic","rarity_relic","rarity_legendary"]
					i=0
					for names in pickup_filter_child_names:
						objectPointer = GetObject(names)
						objectPointer.SetToggleUpEvent(lambda type = 1 << i, ignore = False : self.__OnClickRarityIgnoreButton(type, ignore))
						objectPointer.SetToggleDownEvent(lambda type = 1 << i, ignore = True : self.__OnClickRarityIgnoreButton(type, ignore))
						self.rarityFilterButtonList.append(objectPointer)
						i+=1
			...
			if categoryIndex == 0:#General
				...
				self.arayuzButtonList.append(GetObject("nightmode_on"))
				self.arayuzButtonList.append(GetObject("nightmode_off"))
				self.arayuzButtonList[0].SAFE_SetEvent(self.__OnClickNightMode)
				self.arayuzButtonList[1].SAFE_SetEvent(self.__OnClickNightMode1)
	...
	def UpdateShopRange(self):
		self.__ClickRadioButton(self.shopRangeButtonList, systemSetting.GetShopNamesRange())
	def __OnClickShopRange(self, btnIndex):
		systemSetting.SetShopNamesRange(btnIndex)
		self.UpdateShopRange()
