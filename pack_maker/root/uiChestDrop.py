...
class ChestDropWindow(ui.ScriptWindow):
	...
	def SetOpenAmount(self, amount):
		...
		self.openChestButtonSingle.SetText(uiScriptLocale.Chest_drop_01 % self.openAmount)
		self.openChestButtonMultiple.SetText(uiScriptLocale.Chest_drop_01 % self.openAmount)
	...
	def RefreshItems(self, chestVnum):
		...
		if not self.chestDrop.has_key(self.currentChest):
			chat.AppendChat(chat.CHAT_TYPE_INFO, "Contact with an admin")
			self.Close()
			return
