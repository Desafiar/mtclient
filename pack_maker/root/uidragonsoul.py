...
# C : 단계			B : 강화
# A : 여벌의 번호들...
if app.ENABLE_DRAGONSOUL_THINGS:
	GET_PAGE_INDEX = 0
	GET_DK_INDEX = 0
class DragonSoulWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			if app.ENABLE_DRAGONSOUL_THINGS:
				self.GetChild("DSRefine_Button").SetEvent(ui.__mem_func__(self.ClickRefineButton))
				self.GetChild("DSShop_Button").SetEvent(ui.__mem_func__(self.ClickShopButton))
	...
	def SetInventoryPage(self, page):
		...
		if app.ENABLE_DRAGONSOUL_THINGS:
			...
			GET_PAGE_INDEX = page
			if self.wndDragonSoulRefine != None:
				self.wndDragonSoulRefine.FlushRefineItemSlotEX()
				self.wndDragonSoulRefine.Refresh()
				self.wndDragonSoulRefine.bAutoRefine = False
		self.RefreshBagSlotWindow()
	...
	def __CanUseSrcItemToDstItem(self, srcItemVNum, srcSlotPos, dstSlotPos):
		...
		#chat.AppendChat(1,"%d %d %d %d"%(srcItemVNum,srcSlotPos,dstSlotPos,dstItemVNum))

		if srcItemVNum != 71097:
			return False
	...
	def SelectEmptySlot(self, selectedSlotPos):
		...
		if mouseModule.mouseController.isAttached():
			...
			# elif app.ENABLE_SHOPBOX_SYSTEM and player.SLOT_TYPE_SHOPBOX == attachedSlotType:
				# if player.ITEM_MONEY == attachedItemIndex:
					# # net.SendShopboxCurrencyWithdrawPacket(mouseModule.mouseController.GetAttachedItemCount(), shopbox.SHOPBOX_CURRENCY_GOLD)
					# snd.PlaySound("sound/ui/money.wav")

				# else:
					# net.SendShopboxCheckoutPacket(attachedSlotPos, player.DRAGON_SOUL_INVENTORY, selectedSlotPos)
...
class DragonSoulRefineWindow(ui.ScriptWindow):
	...
	def __init__(self):
		...
		if app.ENABLE_DRAGONSOUL_THINGS:
			self.currentFirstItem=(-1,-1)
			self.currentSecondItem=(-1,-1)
	...
	def __LoadWindow(self):
		...
		try:
			...
			if app.ENABLE_DRAGONSOUL_THINGS:
				self.GetChild("DoAllRefineButton").SetEvent(ui.__mem_func__(self.__PressDoAllRefineButton))
	...
	def Destroy(self):
		...
		self.tabButtonDict = None
		if app.ENABLE_DRAGONSOUL_THINGS:
			self.currentFirstItem=0
			self.currentSecondItem=0
			self.bAutoRefine = 0
			self.autoRefineTime =0
	...
	if app.ENABLE_DRAGONSOUL_THINGS:
		def FlushRefineItemSlotEX(self):
			## Item slot settings
			# 원래 인벤의 아이템 카운트 회복
			for invenType, invenPos, itemCount in self.refineItemInfo.values():
				remainCount = player.GetItemCount(invenType, invenPos)
	...
	def __SetItem(self, inven, dstSlotIndex, itemCount):
		...
		if DragonSoulRefineWindow.REFINE_TYPE_STRENGTH == self.currentRefineType:
			if self.__IsDragonSoul(itemVnum):
				dstSlotIndex = 1
				if app.ENABLE_DRAGONSOUL_THINGS:
					self.currentSecondItem = inven
			else:
				dstSlotIndex = 0
				if app.ENABLE_DRAGONSOUL_THINGS:
					self.currentFirstItem = inven
	...
	if app.ENABLE_DRAGONSOUL_THINGS:
		def __CheckCanRefine(self, vnum, isAuto = False):
			if self.REFINE_TYPE_GRADE == self.currentRefineType:
				return self.__CanRefineGrade(vnum)
			elif self.REFINE_TYPE_STEP == self.currentRefineType:
				return self.__CanRefineStep(vnum, isAuto)
	...
	if app.ENABLE_DRAGONSOUL_THINGS:
		def __CanRefineStep (self, vnum, isAuto = False):
			ds_info = self.__GetDragonSoulTypeInfo(vnum)
	...
	if app.ENABLE_DRAGONSOUL_THINGS:
		...
		def OnUpdate(self):
			if self.bAutoRefine is False:
				return
			if self.autoRefineTime + 0.4 > float(app.GetTime()):
				return
			if self.REFINE_TYPE_STEP == self.currentRefineType:
				slotNumbers = [-1,-1]
				...
				for i in xrange(32):
					...
					if not itemVnum:
						continue
					elif self.__CheckCanRefine(itemVnum, True) == False:
						continue

					slotNumbers[0] = slotNumber
					self.AutoSetItem((player.DRAGON_SOUL_INVENTORY, slotNumber), 1)

					for j in xrange(32):
						slotNumberEx = self.__GetItemPos(j)
						itemVnumEx = player.GetItemIndex(player.DRAGON_SOUL_INVENTORY, slotNumberEx)
						if not itemVnumEx:
							continue
						elif slotNumberEx in slotNumbers:
							continue
						elif self.__CheckCanRefine(itemVnumEx, True) == False:
							continue
						slotNumbers[1] = slotNumberEx
						self.AutoSetItem((player.DRAGON_SOUL_INVENTORY, slotNumberEx), 1)
						break
					if slotNumbers[1] != -1 and slotNumbers[0] != -1:
						break

				if slotNumbers[1] == -1 or slotNumbers[0] == -1:
					self.FlushRefineItemSlotEX()
					self.Refresh()
					self.bAutoRefine = False
					return
				...
				if self.currentRecipe.has_key("fee"):
					if self.currentRecipe["fee"] > player.GetElk():
						chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.DRAGON_SOUL_NOT_ENOUGH_MATERIAL)
						self.bAutoRefine = False
						return
				elif self.currentFirstItem == (-1,-1) or self.currentSecondItem == (-1,-1):
						chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.DRAGON_SOUL_NOT_ENOUGH_MATERIAL)
						self.bAutoRefine = False
						return
				itemVnum = player.GetItemIndex(self.currentSecondItem[0], self.currentSecondItem[1])
				#if not itemVnum:
				#	self.bAutoRefine = False
				#	return
				if self.__CheckCanRefine(itemVnum) == False:
					self.bAutoRefine = False
					return
			else:
				stepIndex = 0
	...
	def RefineSucceed(self, inven_type, inven_pos):
		...
		if app.ENABLE_DRAGONSOUL_THINGS:
			if self.REFINE_TYPE_STRENGTH == self.currentRefineType:
				if self.currentFirstItem != (-1,-1):
					self.AutoSetItem(self.currentFirstItem,1)
				if self.currentSecondItem != (-1,-1):
					self.AutoSetItem(self.currentSecondItem,1)
