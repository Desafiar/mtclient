...
class BuffNPCWindow(ui.ScriptWindow):
	...
	def ClickUpgradeSkillButton(self):
		if self.isActive == True:
			if self.upgradeBookSlotVnum != 0 and self.upgradeBookSlotVnum != 50513:
				...
				# chat.AppendChat(chat.CHAT_TYPE_INFO, "%d - %d - %d" % (int(self.showUpgradeGrandmasterSkill), int(self.canUpgradeGrandmasterSkillList[self.skill_arrow_page]), int(self.skillInfo[self.preview_slot][0])))
				net.SendBuffNPCAction(4, int(self.skillInfo[self.preview_slot][0]), 0, 0, 0)
	...
	def OverInItem(self, index):
		if None != self.toolTip:
			# chat.AppendChat(chat.CHAT_TYPE_INFO,"index: %d, player.BUFF_EQUIPMENT: %d" % (int(index), int(player.BUFF_EQUIPMENT)))
	...
	def RefreshEquipSlotWindow(self):
		# for i in xrange(10):
		for i in xrange(player.BUFF_EQUIPMENT_SLOT_COUNT):
	...
	def SelectEmptySlot(self, selectedSlotPos):
		...
		if isAttached:
			...
			slot_type_list = [
				...
				player.SLOT_TYPE_UPGRADE_INVENTORY,
				player.SLOT_TYPE_BOOK_INVENTORY,
				player.SLOT_TYPE_STONE_INVENTORY,
				player.SLOT_TYPE_CHANGE_INVENTORY,
				player.SLOT_TYPE_COSTUME_INVENTORY
			]
			...
			for i in xrange(len(slot_type_list)):
				if attachedSlotType == slot_type_list[i]:
					attachedCount = mouseModule.mouseController.GetAttachedItemCount()
					net.SendItemMovePacket(SLOT_WINDOW_TYPE[i]["window"], attachedSlotPos, player.BUFF_EQUIPMENT, selectedSlotPos, attachedCount)
