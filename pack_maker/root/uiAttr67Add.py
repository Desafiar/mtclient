...
class Attr67AddWindow(ui.ScriptWindow):
    ...
    def CantAttachToAttrSlot(inven_slot, bAdd):
        ...
        if not item.GetItemType() in (item.ITEM_TYPE_WEAPON, item.ITEM_TYPE_ARMOR, item.ITEM_TYPE_COSTUME , item.ITEM_TYPE_RINGS):
            return False

        if item.GetItemType() == item.ITEM_TYPE_ARMOR and item.GetItemSubType() == item.ARMOR_PENDANT:
            return False
    ...
    def GetMaterialVnum(self, item_level):
        if item_level >= 0 and item_level <= 45:
            return 39070

        if item_level >= 46 and item_level <= 75:
            return 39071

        if item_level >= 76 and item_level <= 105:
            return 39072
    ...
    def __CalculateTotalSuccessPer(self):
        ...
        self.TotalSuccessText.SetText(localeInfo.ATTR_6TH_7TH_TOTAL_SUCCESS_PERCENT % float(percent))
