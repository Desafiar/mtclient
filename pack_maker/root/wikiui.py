...
def IsCanModelPreview(itemVnum):
	...
	if itemType == item.ITEM_TYPE_WEAPON and itemSubType != item.WEAPON_ARROW:
		...
		return True
	elif itemType == item.ITEM_TYPE_COSTUME and (itemSubType == item.COSTUME_TYPE_WEAPON or itemSubType == item.COSTUME_TYPE_BODY or itemSubType == item.COSTUME_TYPE_HAIR or itemSubType == item.COSTUME_TYPE_MOUNT or itemSubType == item.COSTUME_TYPE_PET or itemSubType == item.COSTUME_TYPE_SASH or itemSubType == item.COSTUME_TYPE_SKIN_SASH or (app.ENABLE_MOUNT_SKIN and itemSubType == item.COSTUME_TYPE_MOUNT_SKIN)):
		return True
	return False
...
def SetItemToModelPreview(modelIndex, itemVnum):
	...
	if itemType == item.ITEM_TYPE_COSTUME and (itemSubType == item.COSTUME_TYPE_MOUNT or itemSubType == item.COSTUME_TYPE_PET or (app.ENABLE_MOUNT_SKIN and itemSubType == item.COSTUME_TYPE_MOUNT_SKIN)):
		...
		if itemType == item.ITEM_TYPE_WEAPON:
			...
			if itemSubType == item.COSTUME_TYPE_WEAPON:
				...
				renderTarget.SetArmor(modelIndex, itemVnum)
			elif itemSubType == item.COSTUME_TYPE_SASH or itemSubType == item.COSTUME_TYPE_SKIN_SASH:
				renderTarget.SetArmor(modelIndex, 11299)
