...
class RuhTasi(ui.ScriptWindow):
	...
	def butonlar(self, func):
		if self.aktifslot == None:
			chat.AppendChat(1,"Nejprve vyberte dovednost, kterou chcete rozvíjet!")
			return
	...
	def derecegetir(self):
		...
		self.derece2.SetText("Vaše Hodnost : "+str(point)+" - ")
	...
	def SelectSkill(self, value):
		...
		if skillGrade == 2:
			...
			chat.AppendChat(1,"Nyní mùete zaèít èíst!")
			for i in xrange(0,6):
				if i == value:
			...
			chat.AppendChat(1,"Mùete si vybrat pouze dovednosti velmistra.")
			return
