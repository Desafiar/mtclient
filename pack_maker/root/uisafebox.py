...
class AlmacenMenu(ui.BoardWithTitleBar):
	def __init__(self):
	...
	def __LoadWindow(self):
		self.buttons = [["Almacen"],["Almacen especial"]]

		#self.board = ui.BoardWithTitleBar()
		self.AddFlag("movable")
		self.AddFlag("animate")
		self.SetCenterPosition()
		self.SetTitleName("Menu Almacenes")
...
class SafeboxWindow(ui.ScriptWindow):
	...
	def IsBook(self,vnum):
		if vnum == 50300:
			return True
		return False

	def IsUpgradeItem(self,vnum):
		item.SelectItem(vnum)
		if item.GetItemType() == item.ITEM_TYPE_MATERIAL and item.GetItemSubType() == item.MATERIAL_LEATHER:
			return True
		return False

	def IsStone(self,vnum):
		item.SelectItem(vnum)
		if item.GetItemType() == item.ITEM_TYPE_METIN and item.GetItemSubType() == item.METIN_NORMAL:
			return True
		return False
	...
	def SelectItemSlot(self, selectedSlotPos):
		...
		if app.IsPressed(app.DIK_LCONTROL):
			if self.interface:
				if self.IsBook(selectedItemID):

					if self.interface.AttachItemFromSafebox(selectedSlotPos, player.BOOK_INVENTORY ,safebox.GetItemID(selectedSlotPos)):
						return

					if self.interface.AttachItemSpecialFromSafebox(selectedSlotPos, player.BOOK_INVENTORY ,safebox.GetItemID(selectedSlotPos)):
						return
						
				elif self.IsUpgradeItem(selectedItemID):

					if self.interface.AttachItemFromSafebox(selectedSlotPos, player.UPGRADE_INVENTORY ,safebox.GetItemID(selectedSlotPos)):
						return

					if self.interface.AttachItemSpecialFromSafebox(selectedSlotPos, player.UPGRADE_INVENTORY ,safebox.GetItemID(selectedSlotPos)):
						return

				elif self.IsStone(selectedItemID):

					if self.interface.AttachItemFromSafebox(selectedSlotPos, player.STONE_INVENTORY ,safebox.GetItemID(selectedSlotPos)):
						return

					if self.interface.AttachItemSpecialFromSafebox(selectedSlotPos, player.STONE_INVENTORY ,safebox.GetItemID(selectedSlotPos)):
						return

				else:

					if self.interface.AttachItemFromSafebox(selectedSlotPos, player.INVENTORY ,safebox.GetItemID(selectedSlotPos)):
						return
