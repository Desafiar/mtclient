...
class SwitchbotItemTab(ui.ThinBoardCircle):
	def __init__(self, slot_num, isAttrSlot = False):
		ui.ThinBoardCircle.__init__(self)
		
		self.slot_num = slot_num
		self.isAttrSlot = isAttrSlot
	...
	def __LoadItemTab(self):	
		self.SetSize(ITEM_TAB_WIDTH if not self.isAttrSlot else 38, TOP_PANEL_HEIGHT if not self.isAttrSlot else 5+32+5)
	
		self.statusBar = ui.Bar()
		self.statusBar.SetParent(self)
		self.statusBar.SetSize(ITEM_TAB_WIDTH - 4 if not self.isAttrSlot else 38-4, TOP_PANEL_HEIGHT - 4 if not self.isAttrSlot else 5+32+5-4)
		self.statusBar.SetPosition(2, 2)
		
		self.selectBox = ui.Box()
		self.selectBox.SetParent(self)
		self.selectBox.SetPosition(1, 1)
		self.selectBox.SetSize(ITEM_TAB_WIDTH - 2 if not self.isAttrSlot else 38-2, TOP_PANEL_HEIGHT - 2 if not self.isAttrSlot else 5+32+5-2)
		self.selectBox.SetColor(COLOR_SELECTED)
		
		if not self.isAttrSlot:
		...
		self.itemSlot.ArrangeSlot(0, 1, 3 if not self.isAttrSlot else 1, 32, 32, 0, 0)
		...
		self.itemSlot.SetPosition(0 if not self.isAttrSlot else 2, 5 if self.isAttrSlot else 30)
		self.itemSlot.Show()
	...
	def Destroy(self):
		self.tooltipItem = 0
		self.onSelectEvent = 0
		self.statusBar = 0
		self.itemSlot = 0
		self.selectButton = 0
		self.lastSelectAlpha = 0
		self.lastSelectAlphaDir = 0
		self.selectBox = 0
		self.selectButton = 0
		ui.ThinBoardCircle.Destroy(self)
	...
	def OverInItem(self, overSlotPos):
		...
		if self.IsActive():
			self.tooltipItem.SetSwitchBotItem(itemVnum)
	...
	def RefreshItemSlot(self):	
		if self.IsEmpty():
			...
			if self.isAttrSlot:
				itemCount = 0 if self.IsActive() else itemCount
	...
	def IsActive(self):
		if self.isAttrSlot:
			for j in xrange(switchbot.SLOT_COUNT):
				if switchbot.IsActive(j):
					return True
			return False
...
class SwitchbotAttribute(ui.Window):
	...
	def Destroy(self):
		...
		self.typeInput = 0
		self.typeTextLine = 0
		self.searchTypeButton = 0
		self.deleteAttributeButton = 0
		self.valueInput = 0
		self.valueEditLine = 0
		self.selectedSlot = 0
		self.alternative = 0
		self.index = 0
...
class SwitchbotWindow(ui.BoardWithTitleBar):
	...
	def Destroy(self):
		...
		self.itemTabs = {}
		if self.attrTab:
			self.attrTab.Destroy()
		self.attrTab = None
		for i in xrange(ATTR_SLOT_COUNT):
			itemTab = self.attrDict.get(i, None)
			if not itemTab:
				continue
			itemTab.Destroy()
		self.attrDict = {}
		...
		self.attributeSearchWindow = 0
		self.attrSearchIdx =0
		self.configurationSaveWindow = 0
		self.topPanel = 0
		self.mainPanel = 0
		self.alternativePanel = 0
		self.alternativeTextLine = 0
		self.botPanel = 0
		self.saveButton = 0
		self.startButton = 0
		self.stopButton = 0
	...
	def __LoadWindow(self):
		...
		self.startButton.SetPosition(-self.startButton.GetWidth() / 2 - 10, 0)
		...
		self.stopButton.SetPosition(self.stopButton.GetWidth() / 2 + 20 - 25, 0)
		self.stopButton.SetText(localeInfo.SWITCHBOT_STOP)
		self.stopButton.Show()
		
		attrTab = SwitchbotItemTab(5, True)
		attrTab.SetParent(self.botPanel)
		attrTab.SetPosition((ITEM_TAB_WIDTH + ITEM_TAB_SPACE) * 4 + 35, 0)
		#attrTab.SetOnSelectEvent(ui.__mem_func__(self.SelectSlot))
		#attrTab.OnPlacedItem = self.__OnPlacedItem
		attrTab.Show()
	...
	def SetItemToolTip(self, tooltipItem):
		...
		self.attrTab.SetItemToolTip(tooltipItem)
...
class SearchListWindow(ui.BoardWithTitleBar):
	...
	def Destroy(self):
		...
		self.listItems =0
		self.acceptEvent = 0
		self.acceptArgsType = 0
		self.cancelEvent = 0
		self.searchInput = 0
		self.searchEditLine = 0
		self.scrollBar = 0
...
class ConfigurationWindow(ui.BoardWithTitleBar):
	...
	def Destroy(self):
		...
		self.listBox.RemoveAllItems()
		self.listBox = None
		for key in self.listItems.iterkeys():
			config = self.listItems.get(key, None)
			if not config:
				continue
			config.Destroy()
		self.dict = {}
	...
	def LoadConfigurations(self):
		try:
			f = open(constInfo.USERDATA_DIRECTORY+'switchbot.inf', 'rb')
	...
	def SaveConfigurations(self):
		with open(constInfo.USERDATA_DIRECTORY+'switchbot.inf', 'wb') as f:
