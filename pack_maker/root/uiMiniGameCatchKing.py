...
class CatchKingSimpleWaitingPage(ui.ScriptWindow):
	...
	def __ClickStartButton(self):
		...
		self.startQuestionDialog.SetText1(localeInfo.MINI_GAME_CATCHKING_START_QUESTION_01 % (30000 * self.betNumber, item.GetItemName(), self.betNumber))
		self.startQuestionDialog.SetText2(localeInfo.MINI_GAME_CATCHKING_START_QUESTION_02)
	...
	def __StartAccept(self):
		net.SendMiniGameCatchKingStart(self.betNumber)
...
class CatchKingCard:
	...
	def __CreateCardDestroyEffect(self, parent):
		self.destroyCardEffect = ui.AniImageBox2()
	...
	def __CreateFiveNearEffect(self, parent):
		(x, y) = self.pos
		self.fiveNearEffect = ui.AniImageBox2()
	...
	def __CreateBackgroundImg(self, parent, cardType, clickFunc, clickArg):
		(x, y) = self.pos
		self.backgroundImage = ui.ImageBox2()
	...
	def __CreateBackgroundOverImg(self, parent):
		(x, y) = self.pos
		self.backgroundOverImage = ui.ImageBox2()
	...
	def __CreateBackgroundRowImg(self, parent):
		(x, y) = self.pos
		self.backgroundRowImage = ui.ImageBox2()
	...
	def __CreateArrowImg(self, parent, cardType):
		(x, y) = self.pos
		self.arrowImage = ui.AniImageBox2()
	...
	def __CreateCardCount(self, parent):
		(x, y) = self.pos
		self.cardCountImage = ui.ImageBox2()
...
class CatchKingGamePage(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.myHandCardImage = ui.ImageBox2()
			...
			self.selectedCardImage = ui.ImageBox2()
	...
	def __ClickRewardButton(self):
		...
		net.SendMiniGameCatchKingReward()
	...
	def ClickCard(self, cardNumber):
		#dbg.LogBox("ClickCard")
		if not self.HaveCardInHand():
			#dbg.LogBox("ClickCard - HaveCardInHand")
			return

		if self.isLocked:
			#dbg.LogBox("ClickCard - isLocked")
			return

		net.SendMiniGameCatchKingFieldCardClick(cardNumber)

	def ClickHandCard(self, cardNumber):
		#dbg.LogBox("ClickHandCard")
		if self.HaveCardInHand():
			#dbg.LogBox("ClickHandCard - HaveCardInHand")
			return

		# if not self.handCardList[cardNumber].IsShowArrow():
			# dbg.LogBox("ClickHandCard - handCardList")
			# return

		if self.isLocked:
			#dbg.LogBox("ClickHandCard - isLocked")
			return

		net.SendMiniGameCatchKingDeckCardClick()
