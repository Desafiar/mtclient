...
class OxEventManager(ui.ScriptWindow):
	...
	def Initialize(self):
		...
		for line in lines:
			...
			if tokens[0] != "VNUM" and tokens[0] != "1" and tokens[0] != "2" and tokens[0] != "3":
				try:
					c_szItem = int(tokens[0])
					c_szName = tokens[1].replace("\n","")

					self.c_szItemNew.append(c_szItem)
					self.c_szNameNew.append(c_szName)
				except:
					pass
	...
	def OnUpdateKeyVnum(self):
		...
		for new in xrange(len(self.c_szItemNew)):
			c_szItem = int(self.c_szItemNew[new])
			c_szName = self.c_szNameNew[new]
