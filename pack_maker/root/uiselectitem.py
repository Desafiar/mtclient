...
class SelectItemWindow(ui.ScriptWindow):
	...
	def RefreshSlot(self):
		...
		if app.ENABLE_SPECIAL_STORAGE:
			for i in xrange(player.SPECIAL_PAGE_SIZE):
				...
				itemVNum = getItemVNum(player.STONE_INVENTORY, slotNumber)
				if 0 == itemVNum:
					continue
			...
			for selectWndPos, inventoryPos in self.inventorySlotPosDict.items():
				itemVNum = getItemVNum(player.STONE_INVENTORY, inventoryPos)
				itemCount = getItemCount(player.STONE_INVENTORY, inventoryPos)
	...
	def OverInItem(self, slotIndex):
		if None != self.tooltipItem:
			...
			if app.ENABLE_SPECIAL_STORAGE:
				self.tooltipItem.SetInventoryItem(inventorySlotPos,player.STONE_INVENTORY)
			else:
