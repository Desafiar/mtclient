...
class TipBoard(ui.Bar):
	...
	def __init__(self):
		...
		self.__LoadImage(0, 0, "flag/barra_noticias.tga")
	...
	def __LoadImage(self, x, y, fileName):
		self.imgList = ui.ImageBox()
		self.imgList.SetParent(self)
		self.imgList.AddFlag("not_pick")
		self.imgList.LoadImage(fileName)
		self.imgList.SetPosition(x, y)
		self.imgList.Show()
...
if app.dracaryS_DUNGEON_LIB:
	class MissionBoard(ui.Bar):
		def __init__(self):
			...
			self.startTime = 0
			self.FONT_HEIGHT = 15
			self.LINE_HEIGHT = self.FONT_HEIGHT + 5
			self.STEP_HEIGHT = self.LINE_HEIGHT + 5
			self.LONG_TEXT_START_X = 300
			self.SCREEN_WIDTH = wndMgr.GetScreenWidth()
		...
		def SetMission(self, text):
			...
			self.startTime = app.GetGlobalTimeStamp()+5
			if not self.IsShow():
				self.Show()
		...
		def OnUpdate(self):
			if app.GetGlobalTimeStamp() > self.startTime:
				self.Hide()
				return
