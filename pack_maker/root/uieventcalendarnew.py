...
events_default_data = {
	...
	player.RAMADAN_EVENT:["ramadan_event",localeInfo.RAMADAN_EVENT],
	...
	player.CARD_EVENT:["okey_event",localeInfo.CARD_EVENT],
	player.CATCH_KING_EVENT:["king_event",localeInfo.CATCH_KING_EVENT],
	player.TREASURE_KEY:["keys_event",localeInfo.TREASURE_KEY],

	player.BEGGINER_EVENT:["begginer_event",localeInfo.BEGGINER_EVENT],
}
...
class EventCalendarWindow(ui.BoardWithTitleBar):
	...
	def OnClick(self, eventIndex):
		...
		if not interface:
			return
		if eventIndex == player.CARD_EVENT:
			if interface.wndCardsInfo.IsShow():
				interface.wndCardsInfo.Close()
			else:
				interface.wndCardsInfo.Open()
		elif eventIndex == player.CATCH_KING_EVENT:
			interface.ClickCatchKingEventButton()
