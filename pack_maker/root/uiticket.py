...
CATEGORIES = [
	...
	localeInfo.TICKET_CATEGORY_1,
	localeInfo.TICKET_CATEGORY_2,
	localeInfo.TICKET_CATEGORY_3,
	localeInfo.TICKET_CATEGORY_4,
]
...
class TicketWindow(ui.ScriptWindow):
	...
	def SelectFoundTicket(self, tid = ''):
		...
		for i in xrange(len(constInfo.Tickets['GLOBAL_TICKETS'])):
			if constInfo.Tickets['GLOBAL_TICKETS'][i][0] == id:
				...
				break
		if selected == -1:
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.TICKET_ID_NOT_FOUND % id)
			return
	...
	def SendTicket(self):
		...
		if msg == '' or title == '':
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.TICKET_CAMP_CLEAN)
			return
...
class TicketInfo(ui.ListBoxEx.Item):
	...
	def __CreateTextLine(self, tID, tTitle, priority, tDate, state):		
		...
		imgState.SetUpVisual("d:/ymir work/ui/tickets/%s.tga" % ('onhold','closed','solved')[state-1])
		imgState.SetOverVisual("d:/ymir work/ui/tickets/%s.tga" % ('onhold','closed','solved')[state-1])
		imgState.SetDownVisual("d:/ymir work/ui/tickets/%s.tga" % ('onhold','closed','solved')[state-1])
	...
	class TicketManageMiddle(ui.ScriptWindow):
		class Answer(ui.ScriptWindow):
			...
			def SetContent(self, creator, date, hour, msg):
				self.GetChild('creator_date_value').SetText(localeInfo.TICKET_STATE_BY_DATE % (creator, date, hour))
