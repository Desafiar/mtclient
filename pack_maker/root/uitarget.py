...
class TargetBoard(ui.ThinBoard):
	...
	def __init__(self):
		...
		hpGaugeText = ui.TextLine()
		hpGaugeText.SetParent(hpGauge)
		hpGaugeText.SetHorizontalAlignRight()
		hpGaugeText.SetWindowVerticalAlignCenter()
		hpGaugeText.SetVerticalAlignCenter()
		hpGaugeText.SetOutline()
		hpGaugeText.SetPosition(0, 0)
		hpGaugeText.Hide()
		self.hpGaugeText = hpGaugeText
		...
		if localeInfo.IsARABIC():
			...
			if app.ENABLE_SEND_TARGET_INFO:
				...
				infoButton.SetUpVisual("flag/info1.tga")
				infoButton.SetOverVisual("flag/info2.tga")
				infoButton.SetDownVisual("flag/info3.tga")
	...
	def Close(self):
		self.__Initialize()
		self.Hide()
		# if app.ENABLE_SEND_TARGET_INFO: #Old funcion
			# self.infoButton.showWnd.Close()
	...
	def SetEnemyVID(self, vid):
		...
		if app.ENABLE_SEND_TARGET_INFO:
			...
			self.infoButton.SetPosition(50, 13)
	...
	def SetHP(self, hpPercentage):
		if not self.hpGauge.IsShow():
			self.SetSize(240 + 7 * self.nameLength, self.GetHeight())
			self.name.SetPosition(23, 13)
			self.name.SetWindowHorizontalAlignLeft()
			self.name.SetHorizontalAlignLeft()
			self.hpGaugeText.Show()
